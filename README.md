...
- 调度器是单独运行的 `main.py`。
- Master 会为每台 Node 维护一条常驻 SSH 连接，执行 `hostname`、`nvidia-smi`、`ps` 等命令收集状态。
- 任务满足条件后，Master 会通过常驻 SSH 连接在目标 Node 上启动任务，但任务本身会脱离 SSH 会话继续运行。
- 运行日志会先写到 Node 上的任务运行目录，再持续同步回 Master 本地的 `running_log` 目录，后台查看入口不变。
...
```bash
python -m pip install django django-simpleui paramiko
```
...
- Git
- OpenSSH Client（仅用于手动排查 SSH，不是调度器运行必需）
...
原因是日志、私钥和数据库文件都按相对路径组织，使用简洁路径更方便排查与做计划任务。
...
任务运行后，可以在 `GPU任务运行记录` 中查看状态和日志。当前版本中，任务会先在 Node 上脱离 SSH 会话运行，再由调度器持续把远端日志同步回 Master 本地。

### 4. conda 任务写法

由于任务会在 Node 上以远端 bash 脚本的方式启动并脱离 SSH 会话继续运行，直接写：
