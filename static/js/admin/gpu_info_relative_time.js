(function () {
    function formatRelativeTime(epochMs, mode) {
        var suffix = mode === 'duration' ? '' : '前';
    ...
    function updateRelativeTimes() {
        ...
        nodes.forEach(function (node) {
            ...
            var mode = node.getAttribute('data-relative-mode') || 'ago';
            if (!Number.isFinite(epochMs)) {
                return;
            }
            node.textContent = formatRelativeTime(epochMs, mode);
        });
    }
